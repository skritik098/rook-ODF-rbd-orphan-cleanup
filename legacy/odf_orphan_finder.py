...
class DataCollector:
    ...
    def _collect_must_gather(self):
        ...
        for pool in self.rbd_pools:
            # Find rbd ls output for this pool
            ...
            for img_name in image_names:
                ...
                # Try to find rbd info file for this image
                ...
                # Try to find rbd snap ls for this image
                snap_file = r._find_file([
                    f"*rbd_snap_ls_{pool}_{img_name}*json*",
                    f"*rbd_snap_ls*{pool}*{img_name}*",
                    f"*snap_ls*{img_name}*",
                ])
                if snap_file:
        ...
        for pool in self.rbd_pools:
            ...
            if trash_file:
                ...
                trash_data = r._parse_json_safe(raw) or []
                for t in trash_data:
                    if isinstance(t, dict):
                        ...
                        # Try to find detailed info for this trash entry
    ...
    def _parse_rbd_info_text(raw: str) -> Optional[dict]:
        """Parse non-JSON rbd info output."""
        info = {}
